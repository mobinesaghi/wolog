...
@section('style')
    ...
    /*radiiiiiiiiiiiiiiio*/
        ...
        .radio-input {
            ...
            overflow: hidden;
            border: 1px solid rgba(145, 145, 145, 0.34);
        }
        ...
        .radio-input label:has(input:checked) {
            background-color: rgba(11, 0, 0, 0.13);
        }
        .familyForm input{
            float: left;
...
@section('content')
    ...
    <div class="container"
        ...
        <form class="form" id="formUsers" action="{{url('make/host')}}" method="post" style="text-align: left">@csrf
            <p class="title text-center" style="font-size: 24px">ایجاد مهمان ها </p>
            ...
            <div class="radio-input" id="family-solo-container"  style="width: 100%;">
                <label>
                ...
                <label>
                    <input type="radio" name="gender" value="child">
                    <span>کودک</span>
                </label>
                <span class="selection"></span>
            </div>
                <div class="flex hidden" id="family-group-container" >
                    <label>
                        <input required="" name="mans" id="mansInput" type="number" class="input"value="0" >
                        <span>تعداد اقایان</span>
                    </label>
                    <label>
                        <input required="" name="womens" id="womenInput" type="number" class="input" value="0">
                        <span>تعداد خانم ها</span>
                    </label>
                    <label>
                        <input required="" name="childs" id="childInput" type="number" class="input" value="0">
                        <span>تعداد کودکان</span>
                    </label>
                </div>
            <div class="" style="display: flex;justify-content: space-around;">
                <button class="submit" style="width: 220px"> ایجاد</button>
                <label class="submit" onclick="toggleFamilyGroup()">خانواده</label>
                <script>
                    function toggleFamilyGroup() {
                        document.getElementById('family-group-container').classList.toggle('hidden');
                        document.getElementById('family-solo-container').classList.toggle('hidden');
                        var formAction = document.getElementById('formUsers').action;

                        if (formAction === '{{ url('make/host') }}') {
                            document.getElementById('formUsers').action = '{{ url('make/familyGroup') }}';
                            document.getElementById('womenInput').value = null;
                            document.getElementById('childInput').value = null;
                            document.getElementById('mansInput').value = null;
                   ;
                        } else if (formAction === '{{ url('make/familyGroup') }}') {
                            document.getElementById('formUsers').action = '{{ url('make/host') }}';
                            document.getElementById('womenInput').value = 0;
                            document.getElementById('childInput').value = 0;
                            document.getElementById('mansInput').value = 0
                        }
                    }
                </script>
            </div>

            <label style="background: #424e54;border-radius: 10px; padding: 12px; color: #fff; text-align: center" onclick="getContacts()">از مخاطبین</label>
        </form>

    </div>
    <div class="container w-full flex justify-center"
         style="letter-spacing: 0;justify-content: center;align-items: center;">
        <div class="DDD" id="formContainer" STYLE="display: none;padding-bottom: 50px;width: 90%">
            <form class="form" action="{{url('make/group')}}" method="post" style="text-align: left">@csrf
                <textarea id="contacts" name="contacts" placeholder="مخاطبین ..." style="text-align: right;"></textarea>

                <button type="submit" onclick="return confirmSubmit()"
                        style="background: #424e54;border-radius: 10px; padding: 13px; color: #fff; text-align: center">
                    ایجاد از لیست بالا
                </button>
            </form>
        </div>
                ...
                <table class="min-w-full bg-white shadow-md rounded-lg table-auto">
                    ...
                    @foreach($user->Users->sortBy('to') as $item)
                        <tr class="hover:bg-red-100 cursor-pointer"
                            @if($item->from >2000 and $item->DFrom)
                                style="background: rgba(212,177,6,0.4)"
                            ...
                            <td class="py-4 px-6 text-sm text-gray-900 whitespace-nowrap text-right"
                                ...
                                <div class="" style=" display: flex">
                                    ...
                                    <div style="font-size: 12px;font-weight: 500; transform: rotate(270deg);">
                                        {{ $item->gender == 'man' ? '(آقا)' : ($item->gender == 'boy' ? '(پسر)' : ($item->gender == 'women' ? '(خانم)' : '(کودک)')) }}
                                    </div>
                                </div>
                            </td>
...
@section('script')
    <script>
        ...
        function closeModal() {
            ...
            document.getElementById('family-modal-container').classList.add('hidden')
        }
        document.getElementById('modal-container').addEventListener('click', (e) => {if (e.target === document.getElementById('modal-container')) {closeModal()}})
        document.getElementById('family-modal-container').addEventListener('click', (e) => {if (e.target === document.getElementById('family-modal-container')) {closeModal()}})
