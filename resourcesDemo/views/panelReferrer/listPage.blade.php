            ...
            <div class="container" >
                <div class="container" style="letter-spacing: 0;display: flex;justify-content: center;align-items: center;">
                    <form class="form" action="{{url('list_page')}}" method="post" style="text-align: left">@csrf
                        <div class="flex">
                            <label>
                                <input required="" placeholder="" name="from" type="text" class="input">
                                <span>از کد</span>
                            </label>
                            <label>
                                <input required="" placeholder="" name="to" type="text" class="input">
                                <span>تا کد</span>
                            </label>
    ...
    <div class="w-full flex justify-center">
        <div class="overflow-hidden border border-gray-300 rounded-[23px] bg-white max-w-[350px]">
            <div class="" style="display: flex">
                ...
                <button class="p-2  w-full text-right focus:outline-none focus:none " style="padding: 14px;background:var(--primary-color);color:var(--text-color);  width:350px;text-align: center">ارسال پیامک</button>
            </div>
                        ...
                        @foreach(($allUsers) as $item)
                            <tr class="hover:bg-red-100 cursor-pointer" >
                                <td class="py-4 px-6 text-sm text-gray-900 whitespace-nowrap text-center">
                                    <input type="checkbox" name="ids[]" class="user-checkbox" value="{{ $item->id }}" checked>
                                </td>
                                <td onclick="openModal('{{ $item->name }}', '{{ $item->id }}')" class="py-4 px-6 text-sm text-gray-900 whitespace-nowrap text-right" style="color: #000000 !important;font-weight:600 ">{{$item->name}}</td>
                                <td onclick="openModal('{{ $item->name }}', '{{ $item->id }}')" class="py-4 px-6 text-sm text-gray-900 whitespace-nowrap text-center">{{$item->number}}</td>
                                <td onclick="openModal('{{ $item->name }}', '{{ $item->id }}')" class="py-4 px-6 text-sm text-gray-900 whitespace-nowrap">{{$item->to}}</td>
                            </tr>
                        @endforeach
